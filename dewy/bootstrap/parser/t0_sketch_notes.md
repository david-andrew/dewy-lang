...
- [x] **Bounds in field-default checking:** carry the alphabet's declared length bound into `alphabet.length`, so the default proves that it fits `uint8`. This is distinct from supporting defaults syntactically, which already exists.
- [x] **Bound union-valued flow results:** lower `token:Token = match ...` inside `finish`, including branches constructing different concrete token types. The existing support for distributing flow results through returns does not yet handle this local binding (`no udewy flow temporary representation for Token`).
