...
### Word copies and in-place element updates (2026-09-16)

Two findings from a denser stack sampler (an in-process gdb script that
interrupts the compiler every 5 ms and records the stack, about 5,000
samples per build instead of 250): the detach copy loop generated by
`array_copy_storage` compiled to one byte per iteration, and several
compiler-source functions used a copy-modify-writeback idiom on container
elements (`let frame = frames[i]`, mutate `frame`, `frames[i] = frame`),
which under value semantics copies every array of the element's dictionaries
for one insertion. `array_copy_storage` now copies whole words when the
element width allows, and `bind`, `set_read_type`, `record_requirements`
and the `container_state` read-state updates mutate through an indexed place
argument (`helper(@frames[i] ...)`), which both checkers already accept.

| Compiler source | Wall | Frontend | Validation | Init/reach | Lowering | Emission | Backend |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| lazy graphemes | 24.58 / 24.60 s | 8.56 / 8.43 | 2.27 / 2.28 | 1.26 / 1.29 | 3.68 / 3.78 | 1.05 / 1.07 | 6.51 / 6.48 |
| word copies, place updates | 23.31 / 23.42 s | 7.95 / 8.05 | 2.16 / 2.16 | 0.61 / 0.59 | 3.72 / 3.74 | 1.06 / 1.05 | 6.50 / 6.52 |

Frontend explicit copies fell from 626 MB to 267 MB per build. Validation
copies (244 MB) are branch snapshots of analysis state and remain.

The backend phase was also broken down on a quiet machine. The x86-64 route
is not direct: the µDewy tool writes 60 MB of GNU assembler text, then runs
`as` (3.7 s) and `ld` (0.5 s); its own parsing and encoding take about 2.1 s.
The compiler already passes `--no-debug-info`; with DWARF the text is 139 MB
and `as` takes 5.9 s. The assembler is therefore the largest single backend
...
The typed snapshot codec now follows in `semantic/cache_values.dewy`,
generated by `tools/generate_native_cache.py` from `PreludeSnapshot` and its
reachable declared types. The generated source is committed; native builds
and cache operations do not invoke Python. `--check` detects stale generated
output. Module data moved into `module_state.dewy` so the loader and codec
can share it without an import cycle.

All Session fields are retained together with module metadata, path lookup,
and prelude exports. Dictionary/set lookup tables are rebuilt from logical
entries; syntax/HIR/type arena order and binding identities are preserved.
The codec handles field address contracts and canonical bigint values,
including flattened optional integer unions. Invalid tags, duplicate keys,
truncated fields and trailing bytes return a cache miss instead of a partial
snapshot. An unsupported writer variant marks the output failed.

`native_cache_snapshot.dewy` round-trips parsed syntax, polymorphic HIR and
type nodes, generic alias metadata, warnings, large integers and dictionaries
with removed entries. Re-encoding produces identical bytes; subsequent type
and syntax interning reuses the original indices. The fixture returns 42
through hosted and native output on both x86-64 and C. The hosted optional
forwarding checks and snapshot fixture pass five gates in 87.37 seconds;
most of this is the C test build, not cache operation time. Native evidence
