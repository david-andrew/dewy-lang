...
class _ArrayLowering(_ArraySharing):
    ...
    def _analyze_array_call_boundaries(self) -> dict[int, set[ArrayUse]]:
        ...
        for call in self.array_calls:
            ...
            for position, argument, parameter in self._call_array_arguments(
                ...
                safe |= id(argument) in self.forwarded_values.get(id(call), ())
    ...
    def _classify_array_representations(self) -> None:
        ...
        for function in self.function_by_literal.values():
            for binding in placement.local_arrays(function.literal, self.place_loans.nonescaping, self.place_loans.fixed_storage, self.forwarded_values):
