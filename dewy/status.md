...
- *Regions are runtime values, not marks on one stack.* An arena is a small object `Arena = [cursor end chunks ...]` created lazily on first allocation; allocating from a region bumps that region's cursor, growing it by `mmap`ed chunks; releasing a region returns its chunks to a free list for reuse (no `munmap` in the hot path). Separate regions (rather than save/restore marks on one bump stack) are chosen because a growable structure owned by an outer scope must be able to grow *while inner scopes are live* — `xs.push` inside a loop or callee must allocate in `xs`'s region, which a single stack cannot do without copy-outs — and because a region-as-value is exactly the seam for the context allocator: a descriptor's owner may be a user-provided arena or allocator handle rather than the compiler's default.
- *Ownership in the descriptor.* The array descriptor's `owner` word becomes the owning region/allocator; growth and relocation allocate from `owner`, never from "the current scope". Dictionaries' hidden arrays and runtime-sized union members follow the same rule.
- *Default regions.* The module region owns module-level structures (process lifetime, as now). A function whose body allocates gets a *frame region* only when escape analysis proves its structures do not leave the frame; otherwise those structures are owned individually (tier 1). Loop-iteration regions (allocations proven not to escape the iteration) are the second step; explicit user regions the third.
- *Escapes.* Runtime-length results are allocated directly in the *caller's* region or allocator — the caller passes it as a hidden argument, the same way destination-ABI results already pass a result pointer — so nothing is copied twice and the callee's own region can be released. Stores into outer structures are value copies made in the destination's allocator. Whole-value rebinding of runtime-length arrays across scopes waits for the liveness pass.
- *Slices.* (1) Runtime regions + module and frame regions + owner-based growth + caller-region results; (2) loop-iteration regions with escape proofs; (3) explicit regions and the context allocator. Measurement target: a loop that rebuilds a 1000-element array 10 000 times runs in constant memory.
