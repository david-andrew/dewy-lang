...
Milestone B is being finished in bootstrap-need order (see Sequencing). B1b (recursive union members via handles, member-route narrowing, general unions with `undefined`) landed 2026-08-27 — see the B1 bullet. The next slice is **B8 and the B6 remainder** (string building, byte decoding); B1b's own leftovers (`match`, runtime-sized locals as union members, `array<Node>` member positions) are listed in the B1 bullet. Previously the next slice was described as **B1b: recursive and runtime-sized union members** — the representation is "handle to storage of the member's representation" with the allocation site pluggable (the process-lifetime arena is the first tier, never baked into the layout), plus the three-member `int64|string|undefined` parameter/result limit. A compiler is a recursive AST of sum types, so this is the single biggest bootstrap blocker; `match` syntax is designed alongside it or deferred in favor of `is?` chains.
...
- [ ] **B8. String building.** Join and a builder on top of A1's materialized interpolation. Combining two strings is interpolation (`"{s1}{s2}"`); this slice is the loop-built case. `+` is not concatenation — see the combining-sequences decision.
