...
#define test(ID, cmd)                                                                                                  \
    {                                                                                                                  \
        ...
        for (int i = 0; i < argc; i++)                                                                                 \
            if (strcmp(argv[i], #cmd) == 0) test_##ID = true;                                                          \
        if (test_##ID)                                                                                                 \
        {                                                                                                              \
...
int main(int argc, char* argv[])
{
    ...
    test(vect, -v) test(dict, -d) test(set, -s) test(misc, --misc) test(charset, -c) test(metascanner, -m)
...
#define assert_contains(S, v, r)                                                                                       \
    ...
    assert_contains(S0, 5, true) assert_contains(S0, 4738, true) assert_contains(S0, 10, true)
        assert_contains(S0, 13, true) assert_contains(S0, 42, false)
    ...
    assert_contains(S1, 5, false) assert_contains(S1, 4738, false) assert_contains(S1, 10, true)
        assert_contains(S1, 13, true) assert_contains(S1, 42, true) assert_contains(S1, 546876, true)
    ...
    assert_contains(S2, 5, false) assert_contains(S2, 4738, false) assert_contains(S2, 10, true)
        assert_contains(S2, 13, true) assert_contains(S2, 42, false) assert_contains(S2, 546876, false)

        // check the values from the union
        assert_contains(S3, 5, true) assert_contains(S3, 4738, true) assert_contains(S3, 10, true)
            assert_contains(S3, 13, true) assert_contains(S3, 42, true) assert_contains(S3, 546876, true)
...
int metascanner_tests()
{
    ...
    char* source;
    read_file("../../tests/test_syntax.dewy", &source);
    char* head = source;
...
int charset_tests()
{
    ...
    printf("\nanyset diff test\n");
    charset* anyset = new_charset();
    charset_add_range(anyset, (urange){.start = 0, .stop = 0x10FFFF});
    charset* newline = new_charset();
    charset_add_char(newline, '\n');
    charset* anydiff = charset_diff(anyset, newline);
    charset_repr(anyset);
    printf("(-)\n");
    charset_repr(newline);
    printf("  = ");
    charset_repr(anydiff);
    printf("\n");
