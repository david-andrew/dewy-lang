...
charset* new_charset()
{
    ...
    *s = (charset){
        .ranges = calloc(DEFAULT_CHARSET_CAPACITY, sizeof(urange)), .size = 0, .capacity = DEFAULT_CHARSET_CAPACITY};
    return s;
}
 ...
 */
void charset_add_char(charset* s, uint32_t c) { charset_add_range(s, (urange){.start = c, .stop = c}); }
...
{
    if (s->size == s->capacity) { charset_resize(s, s->size * 2); }
    s->ranges[s->size] = r;
    s->size++;
}
 ...
 */
size_t charset_size(charset* s) { return s->size; }

/**
 * returns the number of unicode ranges the character set can hold
 */
size_t charset_capacity(charset* s) { return s->capacity; }
...
{
    uint64_t length = 0;
    for (int i = 0; i < s->size; i++) { length += s->ranges[i].stop - s->ranges[i].start + 1; }
    return length;
}
...
charset* charset_clone(charset* s)
{
    ...
    for (int i = 0; i < s->size; i++) { charset_add_range_unchecked(copy, s->ranges[i]); }
 ...
 */
void charset_sort(charset* s) { qsort(s->ranges, s->size, sizeof(urange), urange_compare); }
...
void charset_reduce(charset* s)
{
    ...
    for (int i = 1; i < s->size; i++)
    {
        ...
        if (s->ranges[i].start <= new_ranges[j].stop + 1)
        {
            ...
            if (new_ranges[j].stop < s->ranges[i].stop) { new_ranges[j].stop = s->ranges[i].stop; }
        }
    ...
    if (s->size < s->capacity / 2) { charset_resize(s, s->capacity / 2); }
}
...
charset* charset_union(charset* a, charset* b)
{
    ...
    for (int i = 0; i < a->size; i++) { charset_add_range_unchecked(onion, a->ranges[i]); }
    for (int i = 0; i < b->size; i++) { charset_add_range_unchecked(onion, b->ranges[i]); }
...
bool charset_equals(charset* a, charset* b)
{
    ...
    {
        if (a->ranges[i].start != b->ranges[i].start || a->ranges[i].stop != b->ranges[i].stop) { return false; }
    }
    return true;
}
...
int charset_get_c_index(charset* s, uint32_t c)
{
    ...
    {
        int i = (r + l) / 2;
        if (s->ranges[i].start <= c && c <= s->ranges[i].stop) { return i; }
 ...
 */
bool charset_contains_c(charset* s, uint32_t c) { return charset_get_c_index(s, c) >= 0; }
...
bool charset_contains_charset(charset* superset, charset* subset)
{
    ...
    {
        if (!charset_contains_r(superset, subset->ranges[i])) return false;
    }
    return true;
}
 ...
 */
bool urange_contains_c(urange r, uint32_t c) { return r.start <= c && c <= r.stop; }

/**
 * Returns true if c is a character that needs to be escaped in a charset,
 * i.e. `[`, `-`, or `]`
 */
bool is_charset_escape(uint32_t c) { return c == '[' || c == '-' || c == ']' || c == '\\'; }
...
void charset_repr(charset* s)
{
    ...
    for (int i = 0; i < s->size; i++)
    {
        urange r = s->ranges[i];
        printf("  \\x%X", r.start);
        if (r.start != r.stop) { printf("-\\x%X", r.stop); }
