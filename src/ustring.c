...
uint32_t* ustring_charstar_substr(char* str, int start, int stop)
{
    ...
    for (int i = 0; i < start; i++) { eat_utf8(&str); }
    // copy the substring to our unicode array
    for (int i = 0; i < length; i++) { *ptr++ = eat_utf8(&str); }
 ...
 */
uint64_t ustring_parse_hex(uint32_t* str) { return ustring_parse_base(str, 16, hex_digit_to_value); }

/**
 * Read a decimal string, and convert to an unsigned integer.
 */
uint64_t ustring_parse_dec(uint32_t* str) { return ustring_parse_base(str, 10, dec_digit_to_value); }
...
uint32_t eat_utf8(char** str_ptr)
{
    ...
    else if (b0 >> 5 == 0x06) // 2 byte utf-8 character
    {
        uint8_t b1 = **str_ptr;
        (*str_ptr)++;
        if (b1 >> 6 == 0x02) { return (b0 & 0x1F) << 6 | (b1 & 0x3F); }
    }
    else if (b0 >> 4 == 0x0E) // 3 byte utf-8 character
    {
        ...
        if (b1 >> 6 == 0x02)
        {
            uint8_t b2 = **str_ptr;
            (*str_ptr)++;
            if (b2 >> 6 == 0x02) { return (b0 & 0x0F) << 12 | (b1 & 0x3F) << 6 | (b2 & 0x3F); }
        }
    }
    else if (b0 >> 3 == 0x1E) // 4 byte utf-8 character
    {
        ...
        if (b1 >> 6 == 0x02)
        {
            ...
            if (b2 >> 6 == 0x02)
            {
                uint8_t b3 = **str_ptr;
                (*str_ptr)++;
                if (b3 >> 6 == 0x02) { return (b0 & 0x07) << 18 | (b1 & 0x3F) << 12 | (b2 & 0x3F) << 6 | (b3 & 0x3F); }
            }
        }
    }
...
uint32_t peek_unicode(char** str_ptr, size_t index, size_t* delta)
{
    ...
    uint32_t c;
    for (size_t i = 0; i <= index; i++) { c = eat_utf8(str_ptr_copy); }
 ...
 */
bool is_unicode_escape(uint32_t c) { return unicode_to_escape(c) != c; }
...
uint32_t unicode_to_upper(uint32_t c)
{
    ...
    if (unicode_upper_and_lower(c, &uppercase, &_)) { return uppercase; }
...
uint32_t unicode_to_lower(uint32_t c)
{
    ...
    if (unicode_upper_and_lower(c, &_, &lowercase)) { return lowercase; }
...
/**
 * read in unicode_cases.bin, and find the line for the given codepoint for case conversion.
...
{
#define NUM_UNICODE_SWAPPABLE 2620 // number of unicode characters that have upper/lowercase versions
#define TABLE_WIDTH 3              // 3 uint32's for every entry

    // select file version that matches system's endianess
    bool le = is_system_little_endian();
    char* path = le ? "unicode_cases_le.bin" : "unicode_cases_be.bin";

    // read file into an array
    uint32_t table[NUM_UNICODE_SWAPPABLE * TABLE_WIDTH];
    FILE* f = fopen(path, "rb");
    fread(table, sizeof(uint32_t), sizeof(table) / sizeof(uint32_t), f);
    fclose(f);

    // binary search to find the character we want
    int start = 0;
    int end = NUM_UNICODE_SWAPPABLE - 1;
    while (end >= start)
    {

        // get the current codepoint
        int i = (start + end) / 2;
        uint32_t codepoint = table[i * TABLE_WIDTH];
        ...
        else
        {
            ...
            *lowercase = table[i * TABLE_WIDTH + 1];
            *uppercase = table[i * TABLE_WIDTH + 2];
            return true;
        }
    }
    return false;
