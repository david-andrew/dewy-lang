...
def pack(kind:TokenKind, value:TokenValue, location:Location)->PackedToken:
    ...
    while i < n:
        ...
        if is_alpha(c):
            ...
            if str_left_eq("let", text) or str_left_eq("const", text):
                toks.append(pack(TK_LET, NO_VALUE, start))
            elif str_left_eq("and", text):
                toks.append(pack(TK_AND, NO_VALUE, start))
            elif str_left_eq("or", text):
                toks.append(pack(TK_OR, NO_VALUE, start))
            elif str_left_eq("not", text):# and (i >= n or src[i] != '='):  # one special case since `not=?` is checked after
                toks.append(pack(TK_NOT, NO_VALUE, start))
            elif str_left_eq("true", text):
                toks.append(pack(TK_NUMBER, TRUE_VALUE, start))
            elif str_left_eq("false", text):
                toks.append(pack(TK_NUMBER, FALSE_VALUE, start))
            elif str_left_eq("if", text):
                toks.append(pack(TK_IF, NO_VALUE, start))
            elif str_left_eq("loop", text):
                toks.append(pack(TK_LOOP, NO_VALUE, start))
            elif str_left_eq("else", text):
                toks.append(pack(TK_ELSE, NO_VALUE, start))
            elif str_left_eq("return", text):
                toks.append(pack(TK_RETURN, NO_VALUE, start))
            elif str_left_eq("break", text):
                toks.append(pack(TK_BREAK, NO_VALUE, start))
            elif str_left_eq("continue", text):
                toks.append(pack(TK_CONTINUE, NO_VALUE, start))
            elif i < n and src[i] == '(':
                toks.append(pack(TK_IDENT_CALL, len(text), start))
                i += 1 # consume the '('
            elif i < n and src[i] == '[':
                toks.append(pack(TK_IDENT_INDEX, len(text), start))
                i += 1 # consume the '['
            elif i < n and src[i] == '.':
                toks.append(pack(TK_IDENT_DOT, len(text), start))
                i += 1 # consume the '.'
            elif kindof(toks[-1]) == _TK_COLON:
                toks.pop()
                toks.append(pack(TK_TYPE, len(text), start))
                continue
            elif kindof(toks[-1]) == _TK_FN_COLON:
                toks.pop()
                toks.append(pack(TK_FN_TYPE, len(text), start))
                continue
            else:
                # regular identifier
                toks.append(pack(TK_IDENT, len(text), start))
            continue
        ...
        if i+2 < n and src[i] == '0' and src[i+1] == 'x':
            ...
            while i < n and is_hex(src[i]):
                ...
                i += 1
            toks.append(pack(TK_NUMBER, val, start))
            continue
        ...
        if c == '"':
            ...
            toks.append(pack(TK_STRING, i - start, start))
            continue


        # multi-character tokens
        if str_left_eq("//", src[i:]):
            toks.append(pack(TK_IDIV, NO_VALUE, i))
            i += 2
            continue
        if str_left_eq("=?", src[i:]):
            if kindof(toks[-1]) == TK_NOT:
                toks.pop()
                toks.append(pack(TK_NOT_EQ, NO_VALUE, i))
            else:
                toks.append(pack(TK_EQ, NO_VALUE, i))
            i += 2
            continue
        if str_left_eq(">=?", src[i:]):
            toks.append(pack(TK_GT_EQ, NO_VALUE, i))
            i += 3
            continue
        if str_left_eq("<=?", src[i:]):
            toks.append(pack(TK_LT_EQ, NO_VALUE, i))
            i += 3
            continue
        if str_left_eq(">?", src[i:]):
            toks.append(pack(TK_GT, NO_VALUE, i))
            i += 2
            continue
        if str_left_eq("<?", src[i:]):
            toks.append(pack(TK_LT, NO_VALUE, i))
            i += 2
            continue
        if str_left_eq(":>", src[i:]):
            toks.append(pack(_TK_FN_COLON, NO_VALUE, i))
            i += 2
            continue
        if str_left_eq("=>", src[i:]):
            toks.append(pack(TK_FN_ARROW, NO_VALUE, i))
            i += 2
            continue
        if str_left_eq("|>", src[i:]):
            toks.append(pack(TK_PIPE, NO_VALUE, i))
            i += 2
            continue
        if str_left_eq("<<", src[i:]):
            toks.append(pack(TK_LEFT_SHIFT, NO_VALUE, i))
            i += 2
            continue
        if str_left_eq(">>", src[i:]):
            toks.append(pack(TK_RIGHT_SHIFT, NO_VALUE, i))
            i += 2
            continue
        ...
        i += 1
        if c == "+": toks.append(pack(TK_PLUS, NO_VALUE, i));          continue
        if c == "-": toks.append(pack(TK_MINUS, NO_VALUE, i));         continue
        if c == "*": toks.append(pack(TK_MUL, NO_VALUE, i));           continue
        if c == "%": toks.append(pack(TK_MOD, NO_VALUE, i));           continue
        if c == "~": toks.append(pack(TK_BIT_NOT, NO_VALUE, i));       continue
        if c == "(": toks.append(pack(TK_LEFT_PAREN, NO_VALUE, i));    continue
        if c == ")": toks.append(pack(TK_RIGHT_PAREN, NO_VALUE, i));   continue
        if c == "{": toks.append(pack(TK_LEFT_BRACE, NO_VALUE, i));    continue
        if c == "}": toks.append(pack(TK_RIGHT_BRACE, NO_VALUE, i));   continue
        if c == "[": toks.append(pack(TK_LEFT_BRACKET, NO_VALUE, i));  continue
        if c == "]": toks.append(pack(TK_RIGHT_BRACKET, NO_VALUE, i)); continue
        if c == "=": toks.append(pack(TK_ASSIGN, NO_VALUE, i));        continue
        if c == ":": toks.append(pack(_TK_COLON, NO_VALUE, i));        continue
...
def dump_token(token:PackedToken, src:str):
    kind, value, location = unpack(token)
