 ...
 */
metatoken* metatoken_copy(metatoken* t) { return new_metatoken(t->type, ustring_clone(t->content)); }
...
metatoken_type metatoken_get_matching_pair_type(metatoken_type left)
{
    switch (left)
    {
        ...
        default: printf("ERROR: token type %u has no matching pair type", left); exit(1);
    }
}
...
void metatoken_str(metatoken* t)
{
    switch (t->type)
    {
        ...
        case whitespace:
        case comment: ustring_str(t->content); break;
