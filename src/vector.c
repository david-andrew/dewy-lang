 ...
 */
vect* new_vect() { return new_vect_with_capacity(DEFAULT_VECT_CAPACITY); }
...
vect* new_vect_with_capacity(size_t init_capacity)
{
    ...
    *v = (vect){.head = 0, .size = 0, .capacity = init_capacity, .list = calloc(init_capacity, sizeof(obj*))};
    return v;
}
...
void vect_resize(vect* v, size_t new_size)
{
    ...
    for (int i = 0; i < v->size; i++) { new_list[i] = vect_get(v, i); }
...
void vect_insert(vect* v, obj* item, size_t index)
{
    ...
    if (v->size == v->capacity) { vect_resize(v, v->capacity * 2); }
 ...
 */
void vect_append(vect* v, obj* item) { vect_insert(v, item, v->size); }

/**
 * Add an element to the front of the vect.
 */
void vect_prepend(vect* v, obj* item) { vect_insert(v, item, 0); }

/**
 * Add an element to the end of the vect (i.e. top of the stack).
 */
void vect_push(vect* v, obj* item) { vect_append(v, item); }

/**
 * Remove the last element of the vect (i.e. top of the stack) and return it.
 */
obj* vect_pop(vect* v) { return v->size > 0 ? vect_remove(v, v->size - 1) : NULL; }

/**
 * Return the last element in the vect (i.e. top of the stack) without modifying the vect.
 */
obj* vect_peek(vect* v) { return v->size > 0 ? vect_get(v, v->size - 1) : NULL; }

/**
 * Add an element to the end of the vect (i.e. end of the queue).
 */
void vect_enqueue(vect* v, obj* item) { vect_append(v, item); }

/**
 * Remove the first element of the vect (i.e. start of the queue), and return it.
 */
obj* vect_dequeue(vect* v) { return v->size > 0 ? vect_remove(v, 0) : NULL; }
 ...
 */
void vect_delete(vect* v, size_t index) { obj_free(vect_remove(v, index)); }
...
void vect_reset(vect* v)
{
    ...
    for (size_t i = 0; i < v->size; i++) { obj_free(vect_get(v, i)); }

    // ensure all elements of the list are zeroed out
    for (size_t i = 0; i < v->capacity; i++) { v->list[i] = NULL; }
...
{
    vect* copy = new_vect();
    for (int i = 0; i < vect_size(v); i++) { vect_append(copy, obj_copy(vect_get(v, i))); }
    return copy;
}
...
vect* vect_copy_with_refs(vect* v, dict* refs)
{
    ...
    for (int i = 0; i < vect_size(v); i++)
    {
        ...
        if (!dict_contains(refs, item)) { dict_set(refs, item, obj_copy_with_refs(item, refs)); }
...
{
    for (int i = 0; i < v->size; i++) { obj_free(vect_get(v, i)); }
    free(v->list);
    free(v);
}
 ...
 */
bool vect_equals(vect* left, vect* right) { return vect_compare(left, right) == 0; }

/**
 * Lambda function used by hash_uint_sequence,
 * for retrieving values from the vector as a sequence.
 */
uint64_t vect_hash_getval(void* v, size_t i) { return obj_hash(vect_get((vect*)v, i)); }

/**
 * Hash the sequence of elements in the vector.
 */
uint64_t vect_hash(vect* v) { return hash_uint_lambda_sequence(v, vect_size(v), vect_hash_getval); }
