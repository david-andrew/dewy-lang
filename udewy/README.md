...
- The visible value may be *pending* instead of in `rax`: a local plus a constant offset that was named but not yet loaded, a constant, a load not yet performed, or the flags of the last comparison. `save_value` moves a pending local, constant or load straight into its cache register; a comparison uses a pending local, constant or load as the `cmpq` operand; a load or store through a pending local uses it as the base register (`movq 24(%rbx), %r12`, `movq %r12, 24(%rbx)`); a constant stores directly (`movq $0, %r15`); and `if`, loop and short-circuit tests branch on the pending flags (`jne`, `jge`, ...) instead of materializing `-1`/`0`; `not` on a pending comparison inverts it. Every other operation materializes the value first. A loop test such as `i <? n` is therefore `movq`, `cmpq`, `jge`, and a tag test `__load_i64__(cell + 8) =? 140` is `cmpq $140, 8(%rbx)`, `jne`. While a local is still in memory, a site may hold two instructions in one code line (the slot load, then the operation through `rax`); allocation rewrites it to the one-instruction register form.
