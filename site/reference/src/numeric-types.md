...
A comparison between two terms — bindings, fields, lengths — is also kept as a fact about their *difference*, the one relational fact the analysis holds: under `i <? src.length` the value `src.length - i` is at least 1, under `start <=? end` the value `end - start` is at least 0, and after `a =? b` the difference is exactly 0. So `let rest:uint64 = src.length - i` proves inside the guarded loop, and a span width proves after `if 0 <=? start <=? end` (the lower bounds also keep the subtraction within `int64`). A slice's length is its endpoints' difference read the same way — `src[i..]` under `loop i <? src.length` has length at least 1, so it satisfies a `string<length >? 0>` parameter. The fact drops when either side is assigned (except `i += c` / `i -= c` by a constant, which moves the difference by `c` and keeps the fact while it stays nonnegative — `i <? src.length` then `i += 1` leaves `i <=? src.length`), when the sequence shrinks, and at a join where only one path established it; arithmetic on fixed widths stays at the operands' width and meets the annotated width afterwards, so `let w:uint64 = end - start` is `int64 - int64` followed by the proven cast.
