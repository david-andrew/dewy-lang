...
A value of one integer width meeting another — an abstract `int` or an `int64` length stored into a `uint64`, an `int8` widened to `int64` — is a conversion the bounds analysis must prove in range from the facts it has: the type's own range (widening always passes), a comparison, a length (never negative), or a loop guard. An unproven narrowing is a compile error naming the known range (`let b:int8 = w` for an arbitrary `w:int64`), never a silent wrap. The same holds when the target is a union with one fixed-width integer member such as `uint64?`: the integer becomes that member, with that member's proof. Comparisons between different widths (`i:uint64 <? s.length`) compare in the left operand's width — the right operand takes the same proven cast, so no value is ever reinterpreted.
