# Dewy Language Reference

[About This Reference](index.md)

## Source Language

- [Lexical Structure](lexical-structure.md)
  - [Literals](literals.md)
- [Source Files and Execution](execution.md)
- [Bindings, Initialization, and Scope](bindings-and-scope.md)
- [Values, Copies, and Places](values.md)

## Types and Expressions

- [Types and Conversions](types-and-conversions.md)
  - [Numeric Types](numeric-types.md)
- [Expressions and Evaluation](expressions-and-operators.md)
- [Operators and Precedence](operators-and-precedence.md)
- [Functions and Calls](functions-and-calls.md)
- [Control Flow](control-flow-and-iteration.md)
- [Errors, Exceptions, and Forwarding](errors-and-forwarding.md)
- [Ranges and Iteration](ranges-and-iteration.md)
- [Refinements, Effects, and Safety](refinements-and-effects.md)

## Data Types

- [Strings and Graphemes](strings.md)
- [Arrays and Containers](arrays-and-containers.md)
- [Structural Objects](objects.md)
- [Physical Quantities](physical-quantities.md)

## Programs and Implementations

- [Modules and Imports](modules-and-imports.md)
- [Compile-Time Facilities and Metatags](compile-time.md)
- [µDewy and Host Interoperability](interoperability.md)

## Appendices

- [Design Maturity and Open Questions](design-status.md)
- [Implementation Compatibility](compatibility.md)
