...
NODE = 'let Node:type = [value:int64 next:Node|none]\n'
...
def test_recursive_alias_unfolds_to_an_object_with_a_reference_member() -> None:
    ...
    assert type_to_dewy(value) == '[value:int64 next:Node | none]'
...
def test_non_object_recursion_is_rejected() -> None:
    with pytest.raises(UserError, match='must be an object type'):
        _declared('let Chain:type = <(next:Chain|none):>int64>\n')
...
'''
    ...
    # unnarrowed, `node.next.value` is safe navigation: `int64 | none`,
...
def test_every_spelling_of_the_union_shares_one_member_order() -> None:
    declared = _declared(NODE + 'let f = (a:Node|none):>Node|none => a\n')
    signature = declared['f'].expr.type
    assert isinstance(signature, ty.FunctionType)
    param_members = ty.runtime_union_members(ty.optional(signature.pos_or_kw[0].type)) or ('none', ty.optional_payload(signature.pos_or_kw[0].type))
    node_value = declared['Node'].expr.value
    field_members = ('none', ty.optional_payload(node_value.field('next').type))
...
def test_three_member_union_with_none_crosses_calls() -> None:
    emitted = codegen(SrcFile(None, NODE + '''
let pick = (v:Node|int64|none):>int64 => {
    if v is? Node { return v.value }
    else if v is? int64 { return v }
    return 0
}
let main = ():>int64 => pick([value=5 next=none]) + pick(2) + pick(none)
'''))
