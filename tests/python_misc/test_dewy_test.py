...
def test_fail_is_an_expectation_that_always_fails() -> None:
    ...
    # string literals are hoisted into static byte constants; the message text stays plain
    assert emitted.count('_expect_failed()') == 2 and 'the test reached `$fail`' in emitted
