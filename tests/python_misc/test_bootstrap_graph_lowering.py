...
REFINED_TAG_CASES = [
    ...
    (ROOT / 'tests/fixtures/native_optional_views.dewy').read_text(),
    ...
    (ROOT / 'tests/fixtures/native_literal_field_views.dewy').read_text(),
    ...
    (ROOT / 'tests/fixtures/native_binary_views.dewy').read_text(),
    (ROOT / 'tests/fixtures/native_argument_lifetimes.dewy').read_text(),
...
''')
    ...
    for index, body in enumerate([
        ...
        *(body for body, _ in SCALAR_CASES),
        ...
        'State:type=[input:set<int64> output:set<int64>]\nlet main=():>int64=>{let state=State[set[20 22] set[]] loop value in state.input {state.output.push(value)} let sum:int64=0 loop value in state.output {sum+=value} return sum}',
        ...
        (ROOT / 'tests/fixtures/native_string_lifetimes.dewy').read_text(),
    ]):
