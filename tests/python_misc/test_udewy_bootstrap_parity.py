...
def test_short_circuit_contexts_match_between_compilers(bootstrap_binary, tmp_path, target) -> None:
    if which('cc') is None:
        pytest.skip('cc not installed')
    ...
    for name, compiler in [('hosted', ['python', '-m', 'udewy']), ('native', [str(bootstrap_binary)])]:
        work = tmp_path / name
        work.mkdir()
        binary = _compile_with(compiler, source, target, work)
        assert subprocess.run([str(binary)], check=False, timeout=5).returncode == 0


@pytest.mark.parametrize('target', ['x86_64', 'c'])
def test_cached_operand_execution_and_spills(bootstrap_binary, tmp_path, target):
    source = (REPO_ROOT / 'udewy/tests/test_cached_operands.udewy').read_text()
    for name, compiler in [('hosted', ['python', '-m', 'udewy']), ('native', [str(bootstrap_binary)])]:
        work = tmp_path / name
        work.mkdir()
        binary = _compile_with(compiler, source, target, work)
